let jogos = [

    {
        img: {
            src: "https://shared.fastly.steamstatic.com/store_item_assets/steam/apps/632470/header.jpg?t=1727256511",
            alt: "Capa do jogo disco elisyum - the final cut"
        },
        infos: {
            name: "Disco Elysium - The Final Cut",
            price: "R$ 75,49",
            genre: ["rpg", "adventure"],
            theme: "Identidade, Política, Existencialismo, Amnésia",
            href: "https://store.steampowered.com/app/632470/Disco_Elysium__The_Final_Cut/"
        }
    },
    {
        img: {
            src: "../assets/images/capasJogos/celeste.png",
            alt: "capa do jogo celeste"
        },
        infos: {
            name: "Celeste",
            price: "R$ 59,99",
            genre: ["platformer", "action", "adventure"],
            theme: " Ansiedade e Autoaceitação.",
            href: "https://store.steampowered.com/app/504230/Celeste/"
        }
    },
     {
        img: {
            src: "../assets/images/capasJogos/omori.jpg",
            alt: "capa do jogo omori"
        },
        infos: {
            name: "OMORI",
            price: "R$ 37,99",
            genre: ["rpg", "adventure"],
            theme: "Trauma, Luto, Culpa e Saúde Mental.",
            href: "https://store.steampowered.com/app/1150690/OMORI/"
        }
    },
    {
        img: {
            src: "../assets/images/capasJogos/expedition33.jpg",
            alt: "texto alternativo da imagem"
        },
        infos: {
            name: "Clair Obscur Expedition 33",
            price: "R$ 239,00",
            genre: ["rpg", "action", "adventure"],
            theme: "Luto, Mortalidade e a Busca por Propósito.",
            href: "https://store.steampowered.com/app/1903340/Clair_Obscur_Expedition_33/"
        }
    },
      {
        img: {
            src: "../assets/images/capasJogos/GRIS.jpg",
            alt: "texto alternativo da imagem"
        },
        infos: {
            name: "Gris",
            price: "R$ 49,99",
            genre: ["adventure"],
            theme: "Luto, Estágios da Dor, Resiliência",
            href: "https://store.steampowered.com/app/683320/GRIS/"
        }
    },  {
        img: {
            src: "../assets/images/capasJogos/Outer Wilds.jpg",
            alt: "texto alternativo da imagem"
        },
        infos: {
            name: "Outer Wilds",
            price: "R$ 67,99",
            genre: ["action", "adventure"],
            theme: "Medo do Fim, Ciclo Vicioso, Curiosidade, Legado",
            href: "https://store.steampowered.com/app/753640/Outer_Wilds/"
        }
    },  {
        img: {
            src: "../assets/images/capasJogos/Undertale.jpg",
            alt: "texto alternativo da imagem"
        },
        infos: {
            name: "Undertale",
            price: "R$ 19,99",
            genre: ["rpg"],
            theme: "Consequências das Ações, Moralidade, Amizade",
            href: "https://store.steampowered.com/app/391540/Undertale/"
        }
    },  {
        img: {
            src: "../assets/images/capasJogos/Hollow Knight.jpg",
            alt: "texto alternativo da imagem"
        },
        infos: {
            name: "Hollow Knight",
            price: "R$ 46,99",
            genre: ["action", "adventure"],
            theme: "Perda, Sacrifício, Futilidade, Solidão",
            href: "https://store.steampowered.com/app/367520/Hollow_Knight/"
        }
    },  {
         img: {
            src: "../assets/images/capasJogos/CapaSilksong.jpg",
            alt: "capa do jogo Silksong"
        },
        infos: {
             name: "Hollow Knight: Silksong",
            price: "R$ 59,99",
            genre: ["action", "adventure"],
             theme: "Identidade, Destino, Responsabilidade, Agilidade",
             href: "https://store.steampowered.com/app/1030300/Hollow_Knight_Silksong/"
         }
     },  {
        img: {
            src: "../assets/images/capasJogos/this War of Mine.jpg",
            alt: "texto alternativo da imagem"
        },
        infos: {
            name: "This War of Mine",
            price: "R$ 59,99",
            genre: ["adventure", "simulation"],
            theme: "Sobrevivência, Escolhas Morais em Guerra, Desespero",
            href: "https://store.steampowered.com/app/282070/This_War_of_Mine/"
        }
    },  {
        img: {
            src: "../assets/images/capasJogos/Limbo.jpg",
            alt: "texto alternativo da imagem"
        },
        infos: {
            name: "Limbo",
            price: "R$ 37,00",
            genre: ["action", "adventure"],
            theme: " Medo, Incerteza, Limites da Realidade",
            href: "https://store.steampowered.com/app/48000/LIMBO/"
        }
    },  {
        img: {
            src: "../assets/images/capasJogos/Inside.jpg",
            alt: "texto alternativo da imagem"
        },
        infos: {
            name: "Inside",
            price: "R$ 74,00",
            genre: ["action", "adventure"],
            theme: " Controle, Conformidade, Totalitarismo",
            href: "https://store.steampowered.com/app/304430/INSIDE/"
        }
    },  {
        img: {
            src: "../assets/images/capasJogos/Night in the Woods.jpg",
            alt: "texto alternativo da imagem"
        },
        infos: {
            name: "Night in the Woods",
            price: "R$ 59,99",
            genre: ["adventure"],
            theme: "Crise Existencial, Estagnação, Doenças Mentais",
            href: "https://store.steampowered.com/app/481510/Night_in_the_Woods/"
        }
    },  {
        img: {
            src: "../assets/images/capasJogos/To the Moon.jpg",
            alt: "texto alternativo da imagem"
        },
        infos: {
            name: "To the Moon",
            price: "R$ 27,99",
            genre: ["adventure", "rpg"],
            theme: " Memória, Arrependimento, Amor Incondicional",
            href: "https://store.steampowered.com/app/206440/To_the_Moon/"
        }
    },  {
        img: {
            src: "../assets/images/capasJogos/Hellblade.jpg",
            alt: "texto alternativo da imagem"
        },
        infos: {
            name: "Hellblade: Senua's Sacrifice",
            price: "R$ 99,00",
            genre: ["action", "adventure"],
            theme: "Psicose, Esquizofrenia, Saúde Mental, Mitologia Nórdica",
            href: "https://store.steampowered.com/app/414340/Hellblade_Senuas_Sacrifice/"
        }
    },  {
        img: {
            src: "../assets/images/capasJogos/What Remains of Edith Finch.jpg",
            alt: "texto alternativo da imagem"
        },
        infos: {
            name: "What Remains of Edith Finch",
            price: "R$ 55,99",
            genre: ["adventure"],
            theme: "Destino, Maldição Familiar, Narrativas de Morte",
            href: "https://store.steampowered.com/app/501300/What_Remains_of_Edith_Finch/"
        }
    },  {
        img: {
            src: "../assets/images/capasJogos/The Vanishing of Ethan Carter.jpg",
            alt: "texto alternativo da imagem"
        },
        infos: {
            name: "The Vanishing of Ethan Carter",
            price: "R$ 59,99",
            genre: ["adventure"],
            theme: "Luto, Fantasia, Culpa, Realidade Distorcida",
            href: "https://store.steampowered.com/app/258520/The_Vanishing_of_Ethan_Carter/"
        }
    },  {
        img: {
            src: "../assets/images/capasJogos/Life is Strange.jpg",
            alt: "texto alternativo da imagem"
        },
        infos: {
            name: "Life is Strange",
            price: "R$ 199,00",
            genre: ["Ação", "Aventura"],
            theme: "Tempo, Consequências de Escolhas, Amizade, Sacrifício",
            href: "https://store.steampowered.com/app/1265920/Life_is_Strange_Remastered/"
        }
    },  {
        img: {
            src: "../assets/images/capasJogos/Firewatch.jpg",
            alt: "texto alternativo da imagem"
        },
        infos: {
            name: "Firewatch",
            price: "R$ 59,99",
            genre: ["Aventura"],
            theme: "Solidão, Fuga, Culpa, Relacionamentos Difíceis",
            href: "https://store.steampowered.com/app/383870/Firewatch/"
        }
    },  {
        img: {
            src: "../assets/images/capasJogos/Oxenfree.jpg",
            alt: "texto alternativo da imagem"
        },
        infos: {
            name: "Oxenfree",
            price: "R$ 32,99",
            genre: ["Aventura"],
            theme: "Comunicação, Trauma, Paranormal, Amadurecimento",
            href: "https://store.steampowered.com/app/388880/Oxenfree/"
        }
    },  {
        img: {
            src: "../assets/images/capasJogos/Braind.jpg",
            alt: "texto alternativo da imagem"
        },
        infos: {
            name: "Braid",
            price: "R$ 27,99",
            genre: ["casual", "strategy"],
            theme: "Arrependimento, Tempo, Obsessão, Autoengano",
            href: "https://store.steampowered.com/app/26800/Braid/"
        }
    },  {
        img: {
            src: "../assets/images/capasJogos/Florence.jpg",
            alt: "texto alternativo da imagem"
        },
        infos: {
            name: "Florence",
            price: "R$ 18,99",
            genre: ["adventure", "casual"],
            theme: "Relacionamentos, Rotina, Amadurecimento, Desilusão",
            href: "https://store.steampowered.com/app/1102130/Florence/"
        }
    },
     {
        img: {
            src: "../assets/images/capasJogos/hadesII.jpg",
            alt: "texto alternativo da imagem"
        },
        infos: {
            name: "Hades II",
            price: "R$ 88,99",
            genre: ["action", "rpg"],
            theme: "Luto Familiar, Resiliência Feminina, Desafio Cíclico e Aceitação da Perda.",
            href: "https://store.steampowered.com/app/1145350/Hades_II/"
        }
    },{
        img: {
            src: "../assets/images/capasJogos/SOMA.jpg",
            alt: "texto alternativo da imagem"
        },
        infos: {
            name: "SOMA",
            price: "R$ 88,99",
            genre: ["action", "adventure"],
            theme: "Consciência, Identidade, Existencialismo, O que significa ser humano",
            href: "https://store.steampowered.com/app/282140/SOMA/"
        }
    },{
        img: {
            src: "../assets/images/capasJogos/Kentucky_Route_Zero.jpg",
            alt: "texto alternativo da imagem"
        },
        infos: {
            name: "Kentucky Route Zero",
            price: "R$ 45,99",
            genre: ["action", "rpg"],
            theme: "Dívida, Comunidade, Fracasso Americano, Desespero Quotidiano.",
            href: "https://store.steampowered.com/app/1145350/Hades_II/"
        }
    }

    ];

    // Tornar a lista global para que outros scripts a consumam
    if (!window.jogos) window.jogos = jogos;

    // },  {
    //    img: {
    //         src: "url da imagem",
    //         alt: "texto alternativo da imagem"
    //     },
    //     infos: {
    //         name: "Nome do Jogo",
    //         price: "Preço do Jogo",
    //         genre: ["gênero1", "gênero2"],
    //         theme: "Tema do Jogo",
    //         href: "URL do Jogo"
    //     }
    // }, 